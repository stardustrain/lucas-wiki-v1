...
## 2. Test double
  ...
  `apiCallbackStub`이라는 가상의 callback 함수를 stub으로 만들어 보았다. 내부에서의 정확한 동작은 알 길이 없지만(사실은 알 필요도 없다) 함수 호출 시 1이라는 parameter를 받으면 object를 return하고, 아무것도 받지 않으면 error를 throw한다. 위에서 설명한 것 처럼 이런 모습의 stub을 만들면 fetch를 통한 비동기 액션이라든지, DB에 연결하는 등의 복잡한 부분을 건너뛰고 간단히 테스트할 수 있다. 이미 존재하는 class 내부에 존재하는 메소드도 stubbing이 가능하다.
  ...
  - 와닿지않았던 예제 코드 **(잘 사용하는 방법이 있으면 꼭 알려주시길!)**
  ```javascript
  describe('Test double', () => {
    before(function() {
